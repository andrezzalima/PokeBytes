...
app.post("/api/user", async (req, res) => {
  try {
    if (!req.body) {
      ...
      res.status(400).json({
        message: "Username must be bigger than 3 and at max 30 characters'",
      });
...
app.get("/api/user/:id/pokebag", async (req, res) => {
  try {
    ...
    const collectionUser = await getMongoCollection("user");
    const userFounded = await collectionUser.findOne({
      _id: new ObjectId(req.params.id),
    });
    if (!userFounded) {
      res.status(404).json({ message: "not_found" });
  ...
  try {
    if (!req.params.id) {
      ...
      if (!userFounded) {
        ...
        let tradeFounded = await collectionTrades.findOne({
          idUsuario: new ObjectId(req.params.id),
        });
        if (!tradeFounded) {
          ...
          const collectionTrades = await getMongoCollection("trades");
          tradeFounded = await collectionTrades.findOne({
            _id: new ObjectId(newTradeId),
          });
        }
...
app.get("/api/user/:id/card-trade-refresh/:rarity", async (req, res) => {
  // /:idCard?
  ...
  try {
    if (!req.params.id || !req.params.rarity) {
      ...
      if (!userFounded) {
        ...
        const collectionTrades = await getMongoCollection("trades");
        const tradesUser = await collectionTrades.findOne({
          idUsuario: new ObjectId(req.params.id),
        }); //
        ...
        if (
          sameDay(lastTrade, new Date()) &&
          tradesUser.tryOuts >= MAX_TRY_OUTS_DAY
        ) {
          res
            .status(403)
            .json({ message: "Exceeded number of refresh of the day" });
        } else {
          updatedFields[req.params.rarity] = generateListOfIdPokemonsByRarity(
            pokemons,
            req.params.rarity
          );
          ...
          const updatedTrade = await collectionTrades.findOne({
            idUsuario: new ObjectId(req.params.id),
          });
          for (let i = 0; i < RARITY_POKEMONS.length; i++) {
            let rarity = RARITY_POKEMONS[i];
...
app.patch("/api/card-trades", async (req, res) => {
  try {
    ...
    if (!req.body.id || !req.body.idPokemon || !req.body.idCard) {
      res
        .status(401)
        .json({ message: "Required Fields: 'idUser' 'idPokemon' 'idCard' " });
    } else {
      const collectionUser = await getMongoCollection("user");
      const userFounded = await collectionUser.findOne({
        _id: new ObjectId(req.body.id),
      ...
      if (!userFounded) {
        ...
        const cardFounded = await collectionCarta.findOne({
  ...
  try {
    if (!req.params.id) {
      ...
      if (!userFounded) {
        ...
        let inventarioDoUsuario = await collectionInventario.findOne({
          idUsuario: new ObjectId(req.params.id),
        });

        collectionTrades.deleteOne({ idUsuario: new ObjectId(req.params.id) });
